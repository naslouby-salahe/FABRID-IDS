...
# configs/fabrid.yaml. Pass OVERWRITE=1 to commands that support --overwrite.
...
# Primary matched-budget comparison on N-BaIoT using the configured policy suite.
...
help:
	@printf '%s\n' \
		'Quality' \
		'  make install | format | lint | typecheck | test | test-integration | architecture | smoke | check' \
		'' \
		'Always available' \
		...
		'  make results               # build verified results/ paper bundle (OVERWRITE=1 to rebuild)' \
		'  make finalize              # run final configured analysis and reporting steps' \
		'' \
		'Experiments (add OVERWRITE=1 to replace that experiment’s evidence)' \
...
.PHONY: install format lint typecheck test test-integration architecture smoke check \
	validate status preprocess report results finalize help \
