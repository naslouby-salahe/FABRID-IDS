...
| ID | Section | Atomic requirement | Exact value | Impl. | Verif. | Evidence | Notes |
|---|---|---|---|---|---|---|---|
| DATASET-001 | 21 | N-BaIoT primary dataset identity: 9 devices, ~7.06M rows, 115 features, 5 window sizes | | IMPLEMENTED_UNVERIFIED | NOT_AUDITED | `src/fabrid/config/datasets.yaml` | descriptive only so far |
| CLIENT-001 | 22 | Exactly 9 natural N-BaIoT clients with exact benign row counts and Mirai/BASHLITE availability per table | table in section 22 | IMPLEMENTED_UNVERIFIED | NOT_AUDITED | `src/fabrid/config/datasets.yaml:nbaiot.clients` | |
...
| SPLIT-001 | 24 | Benign split boundaries `i1=floor(0.5n)`, `i2=floor(0.7n)`, `i3=floor(0.8n)` -> TRAIN/FRONTIER/FINAL_CAL/TEST | exact floors | MISSING | NOT_AUDITED | | `src/fabrid/data/partitioner.py` not yet written |
| SPLIT-002 | 25 | Exact per-client split counts match published table | table in section 25 | IMPLEMENTED_UNVERIFIED | NOT_AUDITED | `src/fabrid/config/datasets.yaml:split_counts` | must be verified against computed partitioner output once raw data is read (T-level check) |
