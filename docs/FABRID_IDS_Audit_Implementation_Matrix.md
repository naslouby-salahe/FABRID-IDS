...
| ID | Section | Atomic requirement | Impl. | Verif. | Evidence | Notes |
|---|---|---|---|---|---|---|
| TRAIN-001 | 64,109 Phase 3 | Exactly 10 detector seeds {0..9} trained, no seed removal for poor performance | VERIFIED | VERIFIED | `src/fabrid/config/detector.py:load_detector_seeds`; `results/scores/seed_0`..`seed_9` all persisted (real full-scale run, gitignored, ~7.06M records total across all 10 seeds) | no seed excluded |
| TRAIN-002 | 19 | Persist model/scaler/config/hashes per seed | PARTIAL | PARTIAL | `scripts/run_seed_training.py` persists per-client `ScoreArtifact`s (with `score_sha256`) + a sha256 manifest for all 10 real seeds | model/scaler weights themselves not yet separately persisted to disk (only their scoring output) |
...
| SCORE-003 | 89 | Identical persisted anomaly scores consumed by every policy at a given dataset x seed x client x split coordinate (`score_sha256` equality) | | VERIFIED | VERIFIED | `src/fabrid/audit/score_identity.py`, `tests/audit/test_score_identity.py` (same evidence as TEST-T07) | |
...
| STABILITY-001 | 62 | `Instability_k = 1 - max_j P(alpha_k=alpha_j)` from 500 replicates | | VERIFIED | VERIFIED | `src/fabrid/frontier/stability.py:summarize_client_stability`/`run_allocation_sensitivity`, `tests/frontier/test_stability.py` | replicate count is caller-supplied; not yet run at 500 replicates against real seed data |

## STAT-*

| ID | Section | Atomic requirement | Exact value | Impl. | Verif. | Evidence | Notes |
|---|---|---|---|---|---|---|---|
| STAT-001 | 68 | Experimental unit = detector seed, n=10 paired replications, no row-level pseudo-replication | | PARTIAL | PARTIAL | `src/fabrid/statistics/contrasts.py:Contrast.paired_differences` is structurally one scalar per seed (never row-level); `SeedBudgetResult` is one record per seed | satisfied by construction, but no explicit test/audit asserts "not row-level" as an invariant |
