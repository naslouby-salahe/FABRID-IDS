...
def _run(loaded: LoadedSeedScores) -> tuple[ClientResultRecord, ...]:
    ...
    config = config.model_copy(
        update={
            "scoring": config.scoring.model_copy(update={"calibration_conservatism_factor": 1.0})
        }
    )
...
_PRACTICAL_POLICIES = frozenset(
    policy for policy in AllocationPolicy if policy is not AllocationPolicy.TEST_ORACLE
)
