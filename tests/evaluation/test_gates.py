...
def _gates() -> PracticalGateConfig:
    return PracticalGateConfig(
        ...
        fabrid_minimax=FabridMinimaxGateConfig(
...
def test_fabrid_minimax_gate_requires_absolute_worst_client_and_macro_guardrail() -> None:
    ...
    passing = evaluate_fabrid_minimax_gate(evaluations, guardrail, _gates())
    assert isinstance(passing, AvailableFabridMinimaxGate)
    assert passing.availability is EvidenceAvailability.AVAILABLE
    assert passing.status is GateStatus.PASS
    assert passing.budgets_passing == 3
    assert passing.differences[0].worst_client_recall == pytest.approx(0.85)
    failing = evaluate_fabrid_minimax_gate(
        evaluations,
        tuple(
            _budget_contrast(
                budget_id,
    ...
    assert isinstance(failing, AvailableFabridMinimaxGate)
    assert failing.status is GateStatus.FAIL
    assert failing.budgets_passing == 0


def test_fabrid_minimax_gate_requires_complete_evidence() -> None:
    result = evaluate_fabrid_minimax_gate((), (), _gates())
    assert isinstance(result, UnavailableFabridMinimaxGate)
    assert result.availability is EvidenceAvailability.INSUFFICIENT_EVIDENCE
...
def test_analyze_practical_gates_composes_primary_and_guardrail() -> None:
    ...
    primary = analyze_primary_inference(evaluations, levels, statistics)
    ...
    assert isinstance(analysis.fabrid_minimax, AvailableFabridMinimaxGate)
    assert analysis.fabrid_minimax.status is GateStatus.PASS
    guardrail = analyze_minimax_macro_guardrail(evaluations, levels, statistics)
    assert all(
