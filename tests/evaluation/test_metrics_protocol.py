...
def _evaluate_known_allocation(
    ...
    summary, client_records = evaluate_allocation(
        ...
        tuple(
            artifact
            for pair in zip(scores.calibration_frontier, scores.final_calibration, strict=True)
...
def test_evaluate_allocation_fills_secondary_metrics_and_keeps_primaries() -> None:
    ...
    client_a = _confusion(3, 1, 1, 3)
    client_b = _confusion(3, 1, 1, 3)
    ...
    assert summary.federation_fpr == pytest.approx(0.25)
    assert summary.budget_usage == pytest.approx(0.25 / _KNOWN_BUDGET)
    ...
    assert summary.false_alert_gini == pytest.approx(
        false_alert_gini(
            (
                ClientFalseAlerts(client_id=_KNOWN_CLIENT_A, count=1),
