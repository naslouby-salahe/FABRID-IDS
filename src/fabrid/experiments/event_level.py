...
from fabrid.validation.reproducibility import report_progress
        ...
        for budget_per_client_hour in config.event_gate.budgets_per_client_hour:
            for client_id in clients:
                row = _client_event_budget_row(
                    ...
                    _observations_for_client(observations, client_id),
                    client_id,
                )
                if row is not None:
                    rows.append(row)
...
def prepare_and_run_event_level(context: EventLevelContext) -> tuple[EventMetricRow, ...]:
    ...
    if event_config.dataset_id is DatasetId.GOTHAM:
        clients = load_gotham_numeric_features(event_root, application.datasets.gotham)
        for seed in event_config.detector.seeds:
            _materialize_gotham_event_scores(context.paths, event_config, seed, clients)
    ...
    for seed in event_config.detector.seeds:
        ...
        if not observations:
            continue
        seed_tables.append(_event_level_tables(event_config, (seed,), observations, context.paths))
