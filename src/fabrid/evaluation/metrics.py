...
class ClientResultRecord(BaseModel):
    ...
    calibration_order_statistic_rank: RowCount | None = None
    calibration_strict_exceedance_counts: CalibrationExceedanceLattice | None = None
    calibration_distinct_score_n: RowCount | None
    calibration_tied_score_n: RowCount | None
    calibration_smallest_attainable_rate: Probability | None
    calibration_attained_rate: Probability | None
    calibration_requested_attainment_error: Probability | None
...
@dataclass(frozen=True, slots=True)
class _CalibrationRecordFields:
    order_statistic_rank: RowCount | None
    strict_exceedance_counts: CalibrationExceedanceLattice | None
    distinct_score_count: RowCount | None
    tied_score_count: RowCount | None
    smallest_attainable_rate: Probability | None
    attained_rate: Probability | None
    requested_attainment_error: Probability | None


def _calibration_record_fields(
    reachability: CalibrationReachability | None,
) -> _CalibrationRecordFields:
    if reachability is None:
        return _CalibrationRecordFields(None, None, None, None, None, None, None)
    return _CalibrationRecordFields(
        order_statistic_rank=reachability.order_statistic_rank,
        strict_exceedance_counts=",".join(
            str(count) for count in reachability.strict_exceedance_counts
        ),
        distinct_score_count=reachability.distinct_score_count,
        tied_score_count=reachability.tied_score_count,
        smallest_attainable_rate=reachability.smallest_attainable_nonzero_rate,
        attained_rate=reachability.attained_rate,
        requested_attainment_error=reachability.requested_attainment_error,
    )


def _subtype_threshold_outcomes(
    attack_test: ScorePartitionArtifact,
    threshold: Threshold,
) -> tuple[tuple[SubtypeRecall, SubtypeConfusion], ...]:
    subtype_scores = attack_test.subtype_scores()
    if not subtype_scores:
        raise ValueError("attack test requires at least one subtype")
    outcomes: list[tuple[SubtypeRecall, SubtypeConfusion]] = []
    for item in subtype_scores:
        true_positive = int(np.count_nonzero(item.scores > threshold))
        counts = SubtypeConfusionCounts(
            true_positive=true_positive,
            false_negative=item.scores.size - true_positive,
        )
        outcomes.append(
            (
                SubtypeRecall(subtype=item.subtype, rate=counts.true_positive_rate()),
                SubtypeConfusion(subtype=item.subtype, counts=counts),
            )
        )
...
def evaluate_client_threshold(
    ...
    subtype_outcomes = _subtype_threshold_outcomes(attack_test, threshold)
    subtype_recalls, subtype_confusions = zip(*subtype_outcomes, strict=True)
    macro_recall = client_macro_recall(subtype_recalls)
    calibration_fields = _calibration_record_fields(reachability)
    ...
    for confusion in subtype_confusions:
        ...
        records.append(
            ClientResultRecord(
                ...
                calibration_order_statistic_rank=calibration_fields.order_statistic_rank,
                calibration_strict_exceedance_counts=calibration_fields.strict_exceedance_counts,
                calibration_distinct_score_n=calibration_fields.distinct_score_count,
                calibration_tied_score_n=calibration_fields.tied_score_count,
                calibration_smallest_attainable_rate=calibration_fields.smallest_attainable_rate,
                calibration_attained_rate=calibration_fields.attained_rate,
                calibration_requested_attainment_error=calibration_fields.requested_attainment_error,
...
def evaluate_allocation(
    ...
    final_calibration = calibrate_final_thresholds(
        ...
        finite_sample_confidence=finite_sample_confidence,
    )
