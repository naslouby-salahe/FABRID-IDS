...
from fabrid.evaluation.inference import (
    BudgetContrast,
    PrimaryInference,
    analyze_minimax_macro_guardrail,
)
...
class MinimaxBudgetDifference(BaseModel):
    model_config = ConfigDict(frozen=True)
    budget_id: BudgetId
...
class AvailableFabridMinimaxGate(BaseModel):
    model_config = ConfigDict(frozen=True)
    availability: EvidenceAvailability
    status: GateStatus
    budgets_passing: RowCount
...
class UnavailableFabridMinimaxGate(BaseModel):
    model_config = ConfigDict(frozen=True)
    availability: EvidenceAvailability
    reason: FailureReason
...
def evaluate_fabrid_minimax_gate(
    ...
    minimax = gates.fabrid_minimax
    if len(guardrail) != minimax.total_budgets:
        return UnavailableFabridMinimaxGate(
            availability=EvidenceAvailability.INSUFFICIENT_EVIDENCE,
            reason="FABRID-Minimax gate requires all worst-client and MacroRecall effects",
        )
    worst_by_budget = _policy_metric_by_budget(
        evaluations, AllocationPolicy.FABRID_MINIMAX, _worst_client_recall_of
    )
    if len(worst_by_budget) != minimax.total_budgets:
        return UnavailableFabridMinimaxGate(
            availability=EvidenceAvailability.INSUFFICIENT_EVIDENCE,
            reason="FABRID-Minimax gate requires a minimax worst-client recall at every budget",
        )
    differences = tuple(
    ...
    passing = sum(
        ...
        if difference.worst_client_recall >= minimax.minimum_worst_client_recall
        and difference.macro_recall_percentage_points >= -minimax.maximum_macro_recall_loss
    )
    return AvailableFabridMinimaxGate(
        availability=EvidenceAvailability.AVAILABLE,
        status=_status(passing >= minimax.minimum_passing_budgets),
        budgets_passing=passing,
        total_budgets=minimax.total_budgets,
        differences=differences,
    )
...
def analyze_practical_gates(
    ...
    guardrail = analyze_minimax_macro_guardrail(evaluations, budgets, statistics)
    return PracticalGateAnalysis(
        fabrid_macro=evaluate_fabrid_macro_gate(evaluations, gates),
        fabrid_minimax=evaluate_fabrid_minimax_gate(evaluations, guardrail, gates),
