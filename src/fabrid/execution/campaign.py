...
def _run_matched_budget_seed(
    ...
    oracle_scores = oracle_loaded(stored.loaded)
...
def _run_verified_external_replication(context: ApplicationContext, logger: logging.Logger) -> None:
    ...
    logger.info(
        ...
        context.config.protocol.dataset_id.value,
    )
    started = time.monotonic()
    run_external_campaign(context.config, context.paths)
...
def _finalize_campaign(
    ...
    paths = context.paths
    config = context.config.protocol
    _run_verified_external_replication(context, logger)
...
def run_fabrid_campaign(
    ...
    *,
    ...
    records: list[ClientResultRecord] = []
    for seed in config.seeds:
        logger.info("[RUN] seed %d/%d", seed, len(config.seeds))
        ...
        seed_evaluations, seed_records = _run_enabled_seed_experiments(
