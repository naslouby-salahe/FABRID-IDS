...
_SCORING_IMPLEMENTATION_VERSION = "2"
...
def _scoring_config_digest(config: FabridConfig) -> ArtifactDigest:
    return digest_text(
        (
            _SCORING_IMPLEMENTATION_VERSION,
            str(config.detector.score_batch_size),
            config.scoring.model_dump_json(),
        )
    )
...
def score_with_scorer(
    ...
    development_feature: np.ndarray | None,
) -> np.ndarray:
    ...
    if scorer is ScorerDefinition.AE_RECONSTRUCTION:
        return reconstruction_scores
    if feature_values is None or development_feature is None:
        raise ValueError(f"{scorer.value} requires auxiliary feature values")
    auxiliary = _percentiles(feature_values, development_feature)
    if scorer is ScorerDefinition.AUXILIARY_F75:
        return auxiliary
    return np.maximum(_percentiles(reconstruction_scores, development_scores), auxiliary)
...
def _materialize_seed_scores(
    ...
    for client_index, client_id in enumerate(prepared.population.clients, start=1):
        ...
        _materialize_client_scores(
            prepared, trained, paths, config, dataset_id, seed, device, feature_columns, client_id
        )


def _materialize_client_scores(
    prepared: PreparedFederation,
    trained: TrainedDetectorSeed,
    paths: ArtifactPaths,
    config: FabridConfig,
    ...
    auxiliary_feature = config.scoring.auxiliary_feature
    requires_auxiliary = config.scoring.primary_scorer is not ScorerDefinition.AE_RECONSTRUCTION
    ensemble_feature = None if auxiliary_feature is None else auxiliary_feature.preprocessed_column
    if requires_auxiliary and ensemble_feature is None:
        raise ValueError("configured primary scorer requires an auxiliary feature")
    if ensemble_feature is not None and ensemble_feature not in feature_columns:
        raise ValueError(
            f"auxiliary score feature is absent from the feature manifest: {ensemble_feature}"
        )
    ...
    frontier_scores = score_feature_matrix(
        trained.model, scaler.transform(frontier_features), device, config.detector.score_batch_size
    )
    ...
    development_scores: np.ndarray | None = None
    development_feature: np.ndarray | None = None
    ...
    if ensemble_feature is not None:
        ...
        development_scores = frontier_scores
        development_feature = frontier_feature
    ...
    _persist_benign_score_partitions(
        paths,
        coordinate,
        benign_partitions,
        config.scoring.primary_scorer,
        development_scores,
        development_feature,
    )
    _persist_attack_score_partitions(
        paths,
        prepared,
        trained,
        scaler,
...
def _persist_benign_score_partitions(
    paths: ArtifactPaths,
    coordinate: ScoreCoordinate,
    partitions: tuple[tuple[BenignSplit, np.ndarray, np.ndarray | None], ...],
    scorer: ScorerDefinition,
    development_scores: np.ndarray | None,
    development_feature: np.ndarray | None,
) -> None:
    for split, scores, feature in partitions:
        if development_scores is not None:
            scores = score_with_scorer(
                scorer, scores, feature, development_scores, development_feature
            )
        persist_score_partition(
...
def _persist_attack_score_partitions(
    paths: ArtifactPaths,
    prepared: PreparedFederation,
    trained: TrainedDetectorSeed,
    ...
    development_scores: np.ndarray | None,
    development_feature: np.ndarray | None,
    feature_rows: pl.DataFrame | None,
    ensemble_feature: ColumnName | None,
) -> None:
    for split in (AttackSplit.VALIDATION, AttackSplit.TEST):
        ...
        for subtype in _attack_subtypes(paths, prepared, client_id):
            features = load_preprocessed_attack_partition(
                paths, coordinate.dataset_id, client_id, subtype, split, feature_columns
            )
            ...
            if (
                development_scores is not None
                and development_feature is not None
