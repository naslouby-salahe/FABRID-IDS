...
def _feasible_increment(
    ...
    if delta_utility <= TIGHT_TOLERANCE:
        return None
    efficiency = (
...
def allocate_fabrid_macro(
    ...
    return _allocate_fabrid_macro(
        problem, solver, AllocationPolicy.FABRID_MACRO, minimize_budget=True
    )


def allocate_fabrid_macro_without_rate_minimization(
    problem: AllocationProblem,
    solver: SolverConfig,
) -> OptimizedAllocation:
    return _allocate_fabrid_macro(
        ...
        minimize_budget=False,
    )


def _allocate_fabrid_macro(
    problem: AllocationProblem,
    solver: SolverConfig,
    policy: AllocationPolicy,
    *,
    minimize_budget: bool,
) -> OptimizedAllocation:
    ...
    cost_constraints: tuple[LinearConstraint, ...] = ()
    stage_evidence = [stage_one.evidence]
    if minimize_budget:
        stage_two = solve_milp(
            SolverStage.MACRO_MINIMUM_BUDGET,
            program.cost,
            (one_hot, budget, resolution, utility_floor_cheat_guard),
            integrality,
            bounds,
            solver,
        )
        ...
        cost_constraints = (cost_ceiling,)
        variance_constraints = (cost_ceiling_cheat_guard,)
        stage_evidence.append(stage_two.evidence)
    else:
        variance_constraints = ()
    stage_three = solve_milp(
        SolverStage.MACRO_LOWEST_VARIANCE,
        program.utility_variance / program.client_count,
        (one_hot, budget, resolution, utility_floor_cheat_guard, *variance_constraints),
        integrality,
        bounds,
        solver,
    )
    ...
    stage_four = solve_milp(
        ...
        (one_hot, budget, resolution, utility_floor, *cost_constraints, variance_ceiling),
        integrality,
        bounds,
        solver,
    )
    ...
    return OptimizedAllocation(
        allocation=_selected_allocation(policy, program, selection),
        solver=SolverEvidence(
            status=SolverStatus.OPTIMAL,
            stages=(*stage_evidence, stage_three.evidence, stage_four.evidence),
        ),
    )
...
def allocate_common_rate_macro(problem: AllocationProblem) -> Allocation:
    curves = problem.require_eligible_curves()
    weights = problem.eligible_weights()
    candidates: list[tuple[DetectionUtility, TargetFalsePositiveRate]] = []
    for point_index in range(len(curves.clients[0].points)):
        points = tuple(curve.points[point_index] for curve in curves.clients)
        target_rate = points[0].target_rate
        if any(abs(point.target_rate - target_rate) > TIGHT_TOLERANCE for point in points):
            raise ValueError("common-rate optimization requires aligned candidate rate grids")
        cost = sum(weights.for_client(curve.client_id) * target_rate for curve in curves.clients)
        if (
            target_rate <= problem.maximum_target_rate + TIGHT_TOLERANCE
            and cost <= problem.remaining_budget + TIGHT_TOLERANCE
        ):
            candidates.append((sum(point.utility for point in points) / len(points), target_rate))
    if not candidates:
        raise ValueError("common-rate optimization has no budget-feasible candidate")
    _, selected_rate = min(candidates, key=lambda candidate: (-candidate[0], candidate[1]))
    return Allocation(
        policy=AllocationPolicy.COMMON_RATE_MACRO,
        decisions=tuple(
            AllocationDecision(client_id=curve.client_id, target_rate=selected_rate)
...
def resolution_forbidden_rows(
    target_rates: np.ndarray,
    curves: tuple[ClientUtilityCurve, ...],
    frontier_counts: tuple[ClientRowCount, ...],
    resolution_factor: NonNegativeFloat,
) -> np.ndarray:
    count_by_client = {item.client_id: item.row_count for item in frontier_counts}
    client_count = len(curves)
    candidate_count = len(curves[0].points)
    ...
    for client_index, curve in enumerate(curves):
        frontier_count = count_by_client.get(curve.client_id)
        if frontier_count is None:
            raise KeyError(curve.client_id)
        resolution_floor = resolution_factor / (frontier_count + 1)
...
def allocate_fabrid_cvar(
    ...
    worst_client_count = _cvar_client_count(program.client_count, solver.cvar_tail_fraction)
    ...
    stage_three = solve_milp(
        SolverStage.CVAR_MINIMUM_BUDGET,
    ...
    stage_four = solve_milp(
        SolverStage.CVAR_LOWEST_VARIANCE,
    ...
    return OptimizedAllocation(
        allocation=_selected_allocation(AllocationPolicy.FABRID_CVAR, program, selection),
        solver=SolverEvidence(
            status=SolverStatus.OPTIMAL,
            stages=(
                stage_one.evidence,
