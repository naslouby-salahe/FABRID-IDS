...
class ClientFrontierInputs:
    ...
    def __post_init__(self) -> None:
        ...
        if self.calibration_row_count <= 0:
            raise ValueError("client calibration row count must be positive")
...
class AllocationProblem:
    ...
    def calibration_row_counts(self) -> tuple[ClientRowCount, ...]:
        return tuple(
            ClientRowCount(
                client_id=client.client_id,
                row_count=client.calibration_row_count,
            )
