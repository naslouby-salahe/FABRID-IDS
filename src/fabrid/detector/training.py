...
RoundProgressReporter = Callable[[FederatedRoundCount, FederatedRoundCount], None]
...
def train_federated_autoencoder(
    ...
    seed: DetectorSeed,
    device: torch.device,
    round_progress_reporter: RoundProgressReporter | None = None,
) -> Autoencoder:
    ...
    for round_index in range(1, settings.rounds + 1):
        ...
        if round_progress_reporter is not None:
            round_progress_reporter(round_index, settings.rounds)
    return global_model
