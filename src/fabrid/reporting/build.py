...
def _build_tables(
    ...
    suffix = (_system_overhead_table(summary),)
    if external is None:
...
def _absent_practical_gates(reason: FailureReason) -> PracticalGateAnalysis:
    return PracticalGateAnalysis(
        ...
        fabrid_cvar=UnavailableFabridCvarGate(
            availability=EvidenceAvailability.INSUFFICIENT_EVIDENCE,
            reason=reason,
        ),
        budget_compliance=(),
...
def _copy_campaign_run_artifacts(
    ...
    for experiment_id in ExperimentId:
        ...
        if source.is_dir():
            shutil.copytree(
                source,
                destination_root / experiment_id.value,
                dirs_exist_ok=True,
                ignore=shutil.ignore_patterns(".complete", "*.parquet"),
            )
